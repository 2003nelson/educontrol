...
type ResumenAsistencia = { estudiante_id: string; total: number; presentes: number; ausentes: number; porcentaje: number; ultima_fecha: string | null; ultima_updated_at: string | null; ultima_created_at: string | null }
type RegistroDia       = { fecha: string; estado: string; updated_at: string | null; created_at: string | null }
...
function formatISO(d: Date): string { return d.toISOString().split('T')[0] }
...
function generarSemanas(fechasRegistradas: string[]): Semana[] {
  ...
  while (cursor <= hoy) {
    const fin = new Date(cursor); fin.setDate(fin.getDate() + 4)
    semanas.push({
      inicio: formatISO(cursor), fin: formatISO(fin),
...
function diasDeSemana(inicioISO: string): { iso: string; letra: string }[] {
  return ['L', 'M', 'M', 'J', 'V'].map((letra, i) => {
    const d = new Date(inicioISO + 'T12:00:00'); d.setDate(d.getDate() + i)
...
function ModalToggle({ grado, activando, onConfirmar, onCerrar }: { grado: number; activando: boolean; onConfirmar: () => void; onCerrar: () => void }) {
  ...
  return createPortal(
    <>
      <style>{`
        @keyframes bdI{from{opacity:0}to{opacity:1}} @keyframes bdO{from{opacity:1}to{opacity:0}}
...
function GradoCard({ grado, grupos, onClick, idx }: { grado: number; grupos: Grupo[]; onClick: () => void; idx: number }) {
  ...
  return (
    <>
      <div style={{ background:'white', border:'1px solid #e5e5ea', borderRadius:16, boxShadow:'0 1px 4px rgba(0,0,0,0.06)', transition:'transform 0.15s, box-shadow 0.15s', overflow:'hidden', display:'flex', flexDirection:'column', cursor:'pointer', animation:`cardIn 0.42s cubic-bezier(0.34,1.56,0.64,1) ${idx * 0.07}s both` }}
        onClick={onClick}
        ...
        <div style={{ padding:'1.25rem 1.25rem 1rem', borderBottom:'1px solid #f2f2f7' }}>
          <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'0.75rem' }}>
            <div>
              <p style={{ fontSize:'0.7rem', fontWeight:600, color:'#8e8e93', letterSpacing:'0.05em', textTransform:'uppercase', margin:'0 0 0.25rem' }}>Semestre</p>
              <h3 style={{ fontSize:'1.375rem', fontWeight:700, color:'#1c1c1e', margin:0, fontFamily:'Outfit, sans-serif', lineHeight:1.2 }}>{grado}°</h3>
              <div style={{ display:'flex', alignItems:'center', gap:'0.5rem', marginTop:'0.3rem' }}>
                <p style={{ fontSize:'0.7rem', color:'#8e8e93', margin:0, fontWeight:500 }}>{grupos.length} {grupos.length===1?'grupo':'grupos'}</p>
                <span style={{ width:3, height:3, borderRadius:'50%', background:'#c7c7cc', display:'inline-block' }}/>
                <p style={{ fontSize:'0.7rem', color:'#8e8e93', margin:0, fontWeight:500 }}>{totalAlumnos} alumnos</p>
              </div>
            </div>
        ...
        <div style={{ padding:'0.875rem 1.25rem', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', flexDirection:'column', gap:'0.375rem', flex:1 }}>
            ...
            {grupos.length > 3 && <span style={{ fontSize:'0.7rem', color:'#8e8e93', paddingLeft:'0.875rem' }}>+{grupos.length-3} más</span>}
          </div>
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:3 }}>
      <div style={{ display:'flex', alignItems:'center', gap:4 }}>
        <span style={{ fontSize:'0.6rem', fontWeight:700, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'0.06em', flexShrink:0 }}>Toma</span>
        <span style={{ fontSize:'0.7rem', color:'#475569', fontWeight:500 }}>{fechaToma}</span>
      </div>
      {fueEditado && (
        <div style={{ display:'flex', alignItems:'center', gap:4 }}>
          <span style={{ fontSize:'0.6rem', fontWeight:700, color:'#16a34a', textTransform:'uppercase', letterSpacing:'0.06em', flexShrink:0 }}>Edit.</span>
          <span style={{ fontSize:'0.7rem', color:'#16a34a', fontWeight:500 }}>{formatFechaHora(updatedAt!)}</span>
        </div>
      )}
    </div>
  )
        ...
        <table style={{ width:'100%', borderCollapse:'collapse' }}>
          <thead>
            <tr style={{ borderBottom:'1px solid #f1f5f9', position:'sticky', top:0, background:'white', zIndex:1 }}>
              ...
              {dias.map(d => {
                const sel = d.iso === diaSelec
                return (
                  <th key={d.iso} onClick={() => setDiaSelec(d.iso)}
                    style={{ textAlign:'center', padding:'0.75rem 0.5rem', fontSize:'0.7rem', fontWeight: sel?800:700, color: sel?'#1e3a5f':'#64748b', textTransform:'uppercase', minWidth:36, cursor:'pointer', position:'relative', userSelect:'none' }}>
                    {d.letra}
                    {sel && <div style={{ position:'absolute', bottom:4, left:'50%', transform:'translateX(-50%)', width:16, height:2, borderRadius:9999, background:'#2563eb' }}/>}
                  </th>
                )
              })}
              ...
              <th style={{ textAlign:'left', padding:'0.75rem 1rem', fontSize:'0.65rem', fontWeight:700, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'0.08em', minWidth:180 }}>
                <div style={{ display:'flex', alignItems:'center', gap:5 }}>
                  ...
                  <div style={{ display:'flex', gap:2 }}>
                    {dias.map(d => {
                      const sel = d.iso === diaSelec
                      return (
                        <button key={d.iso} onClick={() => setDiaSelec(d.iso)}
                          style={{ width:18, height:18, borderRadius:5, border: sel?'none':'1px solid #e2e8f0', background: sel?'#2563eb':'white', color: sel?'white':'#94a3b8', fontSize:'0.52rem', fontWeight:700, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', padding:0, lineHeight:1 }}>
                          {d.letra}
                        </button>
                      )
                    })}
            ...
            {alumnos.map((alumno, i) => {
              ...
              return (
                <tr key={alumno.id} style={{ borderBottom:'1px solid #f8fafc' }}
                  ...
                  <td style={{ padding:'0.875rem 1rem' }}><span style={{ fontSize:'0.75rem', color:'#64748b' }}>{alumno.matricula ?? '—'}</span></td>
                  {dias.map(d => {
                    const reg = regMap[d.iso]
                    ...
                    const esDiaSelec = d.iso === diaSelec
                    return (
                      <td key={d.iso} style={{ textAlign:'center', padding:'0.875rem 0.5rem', position:'relative', background: esDiaSelec?'#f8faff':'transparent' }}>
                        <div style={{ display:'inline-flex', alignItems:'center', justifyContent:'center', width:28, height:28, borderRadius:'0.5rem', background: esPresente?'#16a34a':esFalta?'#dc2626':esJustif?'linear-gradient(135deg,#f59e0b,#d97706)':esRetardo?'#7c3aed':'#f1f1f4', border: tieneReg?'none':'1px solid #e5e5ea', cursor: tieneReg?'pointer':'default', outline: esDiaSelec?'2px solid #bfdbfe':'none', outlineOffset:1 }}
                          onClick={e => {
                            if (!tieneReg) return
                            e.stopPropagation()
                        ...
                        {tooltip?.id === tooltipId && (
                          <div onClick={e => { e.stopPropagation(); setTooltip(null) }}
                            style={{ position:'absolute', top:'50%', right:'110%', transform:'translateY(-50%)', background:'#1c1c1e', color:'white', fontSize:'0.7rem', fontWeight:500, padding:'4px 10px', borderRadius:7, zIndex:50, whiteSpace:'nowrap', boxShadow:'0 4px 12px rgba(0,0,0,0.3)', pointerEvents:'auto' }}>
                            {tooltip.texto}
                  ...
                  <td style={{ textAlign:'center', padding:'0.875rem 0.75rem' }}>
                    {(() => {
                      const faltas = dias.filter(d => regMap[d.iso]?.estado === 'falta').length
                      return faltas > 0
...
function AlumnosVista({ grupo, alumnos, loadingAlumnos, volver }: {
  ...
  useEffect(() => {
    ...
    async function cargarAsistencias() {
      ...
      setAsistencias([]); setAsistenciaDiaria({}); setSemanas([]); setSemanaSelec(null)
      ...
      if (asistData && asistData.length > 0) {
        ...
        asistData.forEach(r => {
          ...
          const fechaR   = r.fecha as string
          const updR     = r.updated_at as string | null
          const createdR = r.created_at as string | null
          if (!resumen[r.estudiante_id].ultima_fecha || fechaR > resumen[r.estudiante_id].ultima_fecha!) {
            resumen[r.estudiante_id].ultima_fecha      = fechaR
            resumen[r.estudiante_id].ultima_updated_at = updR
            resumen[r.estudiante_id].ultima_created_at = createdR
          }
        })
        ...
        <div style={{ background:'white', borderRadius:'1rem', border:'1px solid #f1f5f9', padding:'3.5rem 2rem', display:'flex', flexDirection:'column', alignItems:'center', gap:'0.75rem' }}>
          <div style={{ width:44, height:44, borderRadius:'0.875rem', background:'#f1f5f9', display:'flex', alignItems:'center', justifyContent:'center' }}>
            <svg width="20" height="20" fill="none" stroke="#94a3b8" strokeWidth="1.8" viewBox="0 0 24 24">
              <path d="M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="1"/><path d="M9 12h6M9 16h4" strokeLinecap="round"/>
            </svg>
          </div>
            ...
            <>
                    ...
                    {filtrados.map((alumno, i) => {
                      ...
                      return (
                        <tr key={alumno.id} style={{ borderBottom:'1px solid #f8fafc' }}
                          ...
                          <td style={{ padding:'0.875rem 1.25rem' }}><span style={{ fontSize:'0.75rem', color:'#64748b' }}>{alumno.matricula ?? '—'}</span></td>
                          <td style={{ padding:'0.875rem 1.25rem' }}>
              ...
              <div style={{ padding:'0.625rem 1.25rem', borderTop:'1px solid #f1f5f9', background:'#fafafa', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                ...
                {!tieneAsistencias
                  ...
                  : <button onClick={() => setSemanaSlide(true)} style={{ fontSize:'0.72rem', fontWeight:600, color:'#2563eb', background:'#eff6ff', border:'1px solid #bfdbfe', borderRadius:'0.5rem', padding:'3px 10px', cursor:'pointer' }}>
                      Ver por semana →
                    </button>
                }
              </div>
...
export default function SeguimientoPage() {
  ...
  useEffect(() => {
    async function cargar() {
      ...
      const { data: gruposData } = await supabase
        .from('grupos').select('id, numero, grado, ciclo_escolar, activo, plantel_id')
        .eq('activo', true).order('grado').order('numero')
      ...
      const conConteo = await Promise.all(gruposData.map(async g => {
        const { count } = await supabase.from('estudiantes').select('id', { count:'exact', head:true }).eq('grupo_id', g.id).eq('activo', true)
  ...
  const cargarGrupo = useCallback(async (grupo: Grupo) => {
    setLoadingAlumnos(true); setAlumnos([])
    const { data } = await supabase.from('estudiantes').select('id, nombre_completo, matricula').eq('grupo_id', grupo.id).eq('activo', true).order('nombre_completo')
    setAlumnos(data ?? [])
      ...
      <div className="px-4 pb-4 pt-3 flex flex-col" style={{ flex:'1 1 0', minHeight:0, overflowY:'auto', gap:'1rem' }}>
        <div style={{ opacity: visible?1:0, transform: visible?'translateX(0) scale(1)':(slideIn+' scale(0.985)'), transition: visible?'opacity 0.38s cubic-bezier(0.34,1.56,0.64,1),transform 0.38s cubic-bezier(0.34,1.56,0.64,1)':'opacity 0.2s ease,transform 0.2s ease' }}>
          ...
          {vista === 'semestres' && (
            <div style={{ display:'flex', flexDirection:'column', gap:'1rem' }}>
              {loadingGrupos ? (
                ...
                <div style={{ textAlign:'center', padding:'4rem' }}><p style={{ fontSize:'0.875rem', color:'#94a3b8' }}>No hay semestres activos</p></div>
              ) : (
                <>
                  ...
                  return (
                    <button key={grupo.id}
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:'0.5rem' }}>
                        ...
                        <span style={{ fontSize:'0.75rem', color:'#94a3b8' }}>Asignaturas: <strong style={{ color:'#64748b' }}>{asignaturasUnicas.length}</strong></span>
                      </div>
                    </button>
                  )
                })}
          ...
          {vista === 'alumnos' && grupoActivo && (
            <AlumnosVista grupo={grupoActivo} alumnos={alumnos} loadingAlumnos={loadingAlumnos}
