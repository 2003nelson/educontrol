...
export default function SeguimientoPage() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Contenedor principal — scroll aquí para todas las vistas */}
      <div className="px-4 pb-4 pt-3 flex flex-col"
        ...
        {vista==='semestres' && (
          <>
            ...
            {/* Cards semestres — usan flex:1 para llenar hasta abajo */}
            <div className="grid grid-cols-3 gap-5" style={{ flex:'1 1 0', minHeight:0 }}>
              {semestresActivos.map(s => (
                <button key={s.numero} onClick={()=>seleccionarSemestre(s.numero)}
                  ...
                  style={{ background:'white', border:'1px solid #e2e8f0', transition:'all 0.2s ease' }}
                  ...
                  {/* Flecha fija en esquina superior derecha del card */}
                  <div style={{ position:'absolute', top:'1rem', right:'1rem', zIndex:2 }}>
                    <ArrowButton/>
                  </div>
                  <div className="relative flex flex-col p-7" style={{ flex:1 }}>
                    <div className="mb-5">
        ...
        {/* VISTA 3: Alumnos — todo con scroll en el contenedor padre */}
        {vista==='alumnos' && (
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                {materiaSelec && (
                  <button onClick={()=>setMateriaSelec(null)}
                    className="text-xs font-medium transition" style={{ color:'#64748b' }}
                ...
                {MATERIAS.map(m=>{
                  ...
                  return (
                    <button key={m} onClick={()=>toggleMateria(m)}
                      ...
                      style={{
                        padding:'0.5rem 1rem',
                        background:activa?'#1e3a5f':'white', color:activa?'white':'#475569',
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0"
                              style={{ background:'#1e3a5f' }}>{fila.alumno.nombre.charAt(0)}</div>
                            <span className="text-sm font-medium" style={{ color:'#1e3a5f' }}>{fila.alumno.nombre}</span>
                          </div>
                        </td>
                        {filtroPeriodo==='bimestre' && (
                          <td className="px-5 py-3.5">
                            <span className="text-sm font-bold" style={{ color:promedioColor(fila.promedio) }}>{fila.promedio}</span>
                          </td>
                        )}
                        <td className="px-5 py-3.5">
                          <span className="text-sm font-bold" style={{ color:asistenciaColor(fila.asistencia) }}>{fila.asistencia}%</span>
                        </td>
                        <td className="px-5 py-3.5">
                          <span className="text-sm font-semibold" style={{ color:fila.faltas>=5?'#dc2626':'#475569' }}>{fila.faltas}</span>
                        </td>
                        <td className="px-5 py-3.5">
                        ...
                        <tr key={alumno.id} style={{ borderBottom:'1px solid #f8fafc' }} className="hover:bg-slate-50 transition-colors">
                          ...
                          <td className="px-4 py-3 sticky left-8 bg-white">
                            <div className="flex items-center gap-2">
                              <div className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold text-white shrink-0"
                                style={{ background:'#1e3a5f' }}>{alumno.nombre.charAt(0)}</div>
                          ...
                          {alumno.bimestres.map(b=>(
                            <td key={`cal-${b.numero}`} className="px-4 py-3">
                              <span className="text-sm font-bold" style={{ color:promedioColor(b.promedio) }}>{b.promedio}</span>
                            </td>
                          ))}
                          <td className="px-4 py-3">
                            <span className="text-sm font-bold px-2 py-0.5 rounded-lg"
                              style={{ background:promCal>=70?'#f0fdf4':'#fef2f2', color:promedioColor(promCal) }}>{promCal}</span>
                          </td>
                          {alumno.bimestres.map(b=>(
                            <td key={`asis-${b.numero}`} className="px-4 py-3">
                              <span className="text-sm font-bold" style={{ color:asistenciaColor(b.asistencia) }}>{b.asistencia}%</span>
                            </td>
                          ))}
                          <td className="px-4 py-3">
                            <span className="text-sm font-bold px-2 py-0.5 rounded-lg"
                              style={{ background:promAsis>=80?'#f0fdf4':'#fef2f2', color:asistenciaColor(promAsis) }}>{promAsis}%</span>
                          </td>
                        </tr>
                      )
                    })}
                  ...
                  <tfoot>
                    <tr style={{ borderTop:'2px solid #e2e8f0', background:'#f8fafc' }}>
                      ...
                      {[1,2,3].map(b=>{
                        ...
                        return <td key={`foot-cal-${b}`} className="px-4 py-3"><span className="text-xs font-bold" style={{ color:'#1e293b' }}>{avg(vals)}</span></td>
                      })}
                      ...
                      {[1,2,3].map(b=>{
                        ...
                        return <td key={`foot-asis-${b}`} className="px-4 py-3"><span className="text-xs font-bold" style={{ color:'#1e293b' }}>{avg(vals)}%</span></td>
                      })}
