...
const aulasMock: Aula[] = [
  { id: '1', grupo: '101', semestre: 1, materia: 'Matemáticas', alumnos: 32 },
  { id: '2', grupo: '301', semestre: 3, materia: 'Matemáticas', alumnos: 29 },
  { id: '3', grupo: '102', semestre: 1, materia: 'Cálculo',     alumnos: 31 },
]
...
function formatFechaCorta() {
  const hoy = new Date()
  return hoy.toLocaleDateString('es-MX', { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric' })
}

// ─── Fila alumno ──────────────────────────────────────────────────────────────
function FilaAlumno({ alumno, index, onCambiarEstado }: {
  alumno: Alumno
  index: number
  onCambiarEstado: (id: string, estado: EstadoAsistencia) => void
}) {
  return (
    <tr style={{ borderBottom: '1px solid #f8fafc', transition: 'background 0.15s' }} className="hover:bg-slate-50">
      <td className="px-5 py-3 text-xs" style={{ color: '#94a3b8', width: '48px' }}>{index + 1}</td>
          ...
          {(['P', 'A', 'J'] as EstadoAsistencia[]).map(e => {
            ...
            return (
              <button key={e}
                ...
                onMouseEnter={e => { if (!activo) { e.currentTarget.style.background = cfg.bgActivo; e.currentTarget.style.color = 'white'; e.currentTarget.style.transform = 'scale(1.05)' } }}
                onMouseLeave={e => { if (!activo) { e.currentTarget.style.background = cfg.bg; e.currentTarget.style.color = cfg.color; e.currentTarget.style.transform = 'scale(1)' } }}
                title={cfg.label}>
                {e}
              </button>
            )
      ...
      <td className="px-5 py-3">
        {alumno.estado ? (
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full"
            style={{ background: ESTADO_CONFIG[alumno.estado].bg, color: ESTADO_CONFIG[alumno.estado].color, border: `1px solid ${ESTADO_CONFIG[alumno.estado].border}` }}>
        ...
        {vista === 'aulas' && (
          <>
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-semibold" style={{ color: '#1e3a5f' }}>Mis aulas asignadas</p>
                <p className="text-xs mt-0.5" style={{ color: '#94a3b8' }}>{formatFechaHoy()}</p>
              </div>
              <span className="text-xs font-semibold px-2.5 py-1 rounded-full"
            ...
            <div className="grid grid-cols-3 gap-5">
              {aulasMock.map(aula => {
                const col = getColorMateria(aula.materia)
                return (
                  <div key={aula.id}
                  ...
                  >
                    ...
                    <div className="px-5 pt-4 pb-5">
                      ...
                      <button
                        ...
                        style={{ background: `linear-gradient(135deg, ${col.from}, ${col.to})`, color: 'white', border: 'none' }}
            ...
            <div className="bg-white rounded-2xl px-5 py-3 shadow-sm" style={{ border: '1px solid #f1f5f9' }}>
              ...
              <div className="h-2 rounded-full overflow-hidden" style={{ background: '#f1f5f9' }}>
                <div className="h-full rounded-full transition-all duration-500"
                  style={{ width: `${progreso}%`, background: progreso === 100 ? '#16a34a' : '#3b82f6' }} />
              </div>
            </div>
              ...
              <table className="w-full">
                <thead>
                  <tr style={{ borderBottom: '1px solid #f1f5f9', background: '#fafafa' }}>
                    {['#', 'Alumno', 'Asistencia', 'Estado'].map(col => (
                ...
                <tbody>
                  {alumnosFiltrados.map((alumno, i) => (
                    <FilaAlumno key={alumno.id} alumno={alumno} index={i} onCambiarEstado={cambiarEstado} />
                  ))}
                </tbody>
              </table>
              <div className="px-5 py-3 border-t flex items-center justify-between"
                ...
                <div className="flex gap-3">
                  {(['P', 'A', 'J'] as EstadoAsistencia[]).map(e => {
                    ...
                    return (
                      <span key={e} className="flex items-center gap-1 text-xs font-semibold" style={{ color: cfg.color }}>
