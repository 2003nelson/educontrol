...
export default function LoginPage() {
  ...
  async function handleLogin() {
    ...
    try {
      ...
      // Verificar rol desde la tabla usuarios para mayor seguridad
      ...
      if (rol === 'super_admin') {
        ...
        router.push('/docente/grupos')  // ← corregido
      } else {
  ...
  if (splash) {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center"
        ...
        <style>{`
          @keyframes splashIn { from { opacity:0; transform:scale(0.88) translateY(16px); } to { opacity:1; transform:scale(1) translateY(0); } }
          @keyframes dotPulse { 0%,100% { opacity:0.4; transform:scale(0.85); } 50% { opacity:1; transform:scale(1.15); } }
          @keyframes splashPulse { 0%,100% { box-shadow: 0 0 0 0 rgba(59,130,246,0.4), 0 8px 32px rgba(37,99,235,0.35); } 50% { box-shadow: 0 0 0 12px rgba(59,130,246,0), 0 8px 32px rgba(37,99,235,0.35); } }
        `}</style>
        <div style={{ position: 'absolute', inset: 0, background: 'rgba(15,23,42,0.5)', backdropFilter: 'blur(3px)' }}/>
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.25rem', animation: 'splashIn 0.55s cubic-bezier(0.34,1.56,0.64,1)' }}>
          <div style={{ background: 'rgba(255,255,255,0.14)', backdropFilter: 'blur(24px)', borderRadius: '1.75rem', border: '1px solid rgba(255,255,255,0.28)', boxShadow: '0 24px 64px rgba(0,0,0,0.28)', padding: '2.5rem 3rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.125rem' }}>
            <div style={{ width: '64px', height: '64px', borderRadius: '18px', background: 'linear-gradient(135deg,#3b82f6,#2563eb)', display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'splashPulse 2s ease-in-out infinite' }}>
              <span style={{ color: 'white', fontSize: '28px', fontWeight: 700, fontFamily: 'Outfit,sans-serif' }}>E</span>
            </div>
            <div style={{ textAlign: 'center' }}>
              <p style={{ fontSize: '1.375rem', fontWeight: 700, color: 'white', fontFamily: 'Outfit,sans-serif', margin: 0 }}>EduControl</p>
              <p style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.55)', margin: '0.375rem 0 0' }}>Sistema de administración escolar</p>
            </div>
            <div style={{ display: 'flex', gap: '0.4rem', marginTop: '0.25rem' }}>
              {[['#ef4444','rgba(239,68,68,0.4)'],['#f59e0b','rgba(245,158,11,0.4)'],['#22c55e','rgba(34,197,94,0.4)']].map(([color, glow], i) => (
                <div key={i} style={{ width: '8px', height: '8px', borderRadius: '50%', background: color, animation: `dotPulse 1.2s ease-in-out ${i*0.22}s infinite`, boxShadow: `0 0 6px ${glow}` }}/>
              ))}
            </div>
          </div>
          <p style={{ fontSize: '0.7rem', color: 'rgba(255,255,255,0.3)', margin: 0 }}>Dinoti Platforms</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen flex" style={{ background: '#0f172a' }}>
      <style>{`
        ...
        @media (max-width: 768px) {
          ...
          .login-card { animation: fadeInUp 0.6s cubic-bezier(0.22,1,0.36,1) !important; }
          .mobile-logo { display: flex !important; flex-direction: column; align-items: center; gap: 0.5rem; }
          .login-card { margin-top: 6rem !important; }
        }
      `}</style>
            ...
            <div>
              {resetSent ? (
                <div style={{ textAlign: 'center', padding: '1rem 0' }}>
                  ...
                  <button onClick={() => { setOlvidé(false); setResetSent(false); setResetEmail(''); setError('') }} style={{ fontSize: '0.875rem', fontWeight: 600, color: '#3b82f6', background: 'none', border: 'none', cursor: 'pointer' }}>← Volver al inicio de sesión</button>
                </div>
              ) : (
                <>
                  ...
                  {error && <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '0.75rem', padding: '0.75rem 1rem', marginBottom: '1.25rem' }}><p style={{ fontSize: '0.875rem', color: '#dc2626', margin: 0 }}>{error}</p></div>}
                  ...
                  <button onClick={() => { setOlvidé(false); setError('') }} style={{ width: '100%', fontSize: '0.875rem', fontWeight: 500, color: '#64748b', background: 'none', border: 'none', cursor: 'pointer', textAlign: 'center' }}>← Volver al inicio de sesión</button>
                </>
              )}
            </div>
          ) : (
            <div>
              ...
              <div style={{ marginBottom: '1rem' }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input type={showPass ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" autoComplete="current-password" onKeyDown={e => e.key === 'Enter' && handleLogin()}
                    ...
                    onFocus={e => (e.currentTarget.style.borderColor = '#3b82f6')} onBlur={e => (e.currentTarget.style.borderColor = '#e2e8f0')} />
                  <button onClick={() => setShowPass(p => !p)} type="button" style={{ position: 'absolute', right: '1rem', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', padding: 0, display: 'flex', alignItems: 'center' }}>
                    {showPass ? (
