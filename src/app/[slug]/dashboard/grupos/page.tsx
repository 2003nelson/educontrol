...
function GrupoModal({
  ...
  onGuardar: (nombre: string, semestre: number) => Promise<void>
  onCerrar: () => void
  guardando: boolean
}) {
  ...
  async function handleConfirmar() {
    await onGuardar(nombre.trim(), semestre)
    cerrar()
  }
        ...
        <div onClick={e => e.stopPropagation()} style={{ background:'white', borderRadius:'1rem', width:'400px', padding:'2rem', display:'flex', flexDirection:'column', alignItems:'center', boxShadow:'0 20px 60px rgba(0,0,0,0.2)', animation:modalAnim }}>
          ...
          <div style={{ display:'flex', gap:'0.75rem', width:'100%' }}>
            <button onClick={() => setConfirmando(false)} disabled={guardando}
              ...
              onMouseEnter={e => { if (!guardando) e.currentTarget.style.background='#f8fafc' }}
            ...
            <button onClick={handleConfirmar} disabled={guardando}
              ...
              onMouseEnter={e => { if (!guardando) e.currentTarget.style.background='#2563eb' }}
              onMouseLeave={e => { if (!guardando) e.currentTarget.style.background='#1e3a5f' }}>
...
// ─── Modal editar grupo ───────────────────────────────────────────────────────
function ModalEditarGrupo({ grupo, onGuardar, onCerrar, guardando }: {
  grupo: Grupo
  ...
  async function guardar() {
    ...
    await onGuardar(grupo.id, nombre.trim(), semestre)
    cerrar()
  }
          ...
          <div style={{ padding:'1.25rem', display:'flex', flexDirection:'column', gap:'0.875rem' }}>
            ...
            <div style={{ display:'flex', gap:'0.625rem', paddingTop:'0.25rem' }}>
              <button onClick={cerrar} disabled={guardando} style={{ flex:1, padding:'0.625rem', fontSize:'0.8rem', fontWeight:500, borderRadius:'0.75rem', border:'1px solid #e2e8f0', color:'#64748b', background:'white', cursor: guardando ? 'not-allowed' : 'pointer', opacity: guardando ? 0.5 : 1 }}
                onMouseEnter={e=>{ if (!guardando) e.currentTarget.style.background='#f8fafc' }} onMouseLeave={e=>(e.currentTarget.style.background='white')}>Cancelar</button>
              <button onClick={guardar} disabled={guardando} style={{ flex:1, padding:'0.625rem', fontSize:'0.8rem', fontWeight:600, borderRadius:'0.75rem', border:'none', background: guardando ? '#94a3b8' : '#1e3a5f', color:'white', cursor: guardando ? 'not-allowed' : 'pointer', transition:'background 0.15s' }}
                onMouseEnter={e=>{ if (!guardando) e.currentTarget.style.background='#2563eb' }} onMouseLeave={e=>{ if (!guardando) e.currentTarget.style.background='#1e3a5f' }}>
...
function ModalCargaAlumnos({ grupo, onCargar, onCerrar, cargando }: {
  grupo: Grupo
  onCargar: (grupoId: string, filas: AlumnoFila[]) => Promise<void>
  onCerrar: () => void
  cargando: boolean
}) {
  ...
  const listo = filasPaste.length > 0

  async function cargarYCerrar() {
    const resultado: AlumnoFila[] = filasPaste.map((f, i) => ({ 
      id: `f-${Date.now()}-${i}`, 
      matricula: f.matricula, 
      nombre: f.nombre 
    }))
    await onCargar(grupo.id, resultado)
    setCargado(true)
        ...
        <div onClick={e=>e.stopPropagation()} style={{ background:'white', borderRadius:'1.5rem', width:'900px', maxWidth:'calc(100vw - 2rem)', height:'520px', display:'flex', flexDirection:'column', boxShadow:'0 32px 80px rgba(0,0,0,0.22)', pointerEvents:'all', overflow:'hidden', animation: cerrando?'cgO 0.38s cubic-bezier(0.34,1.56,0.64,1) forwards':'cgI 0.46s cubic-bezier(0.34,1.56,0.64,1)' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'1.25rem 1.75rem', borderBottom:'1px solid #f1f5f9', flexShrink:0 }}>
            <div>
              <h2 style={{ fontSize:'1rem', fontWeight:700, color:'#1e3a5f', margin:0, fontFamily:'Outfit,sans-serif' }}>
                Cargar alumnos — Grupo {grupo.nombre}
              </h2>
              <p style={{ fontSize:'0.75rem', color:'#94a3b8', margin:'0.2rem 0 0' }}>
                Pega desde Excel — soporta matrícula + nombre en columnas separadas
              </p>
            </div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'5fr 7fr', flex:'1 1 0', minHeight:0, overflow:'hidden' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', minHeight:0 }}>
              <div style={{ padding:'1.25rem 1.5rem 0.625rem', flexShrink:0, display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                <p style={{ fontSize:'0.65rem', fontWeight:700, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'0.08em', margin:0 }}>Vista previa</p>
                <span style={{ fontSize:'0.72rem', fontWeight:600, padding:'0.15rem 0.5rem', borderRadius:'9999px', background: listo?'#eff6ff':'#f1f5f9', color: listo?'#2563eb':'#94a3b8' }}>
                  {filasPaste.length} alumno{filasPaste.length!==1?'s':''}
                </span>
              </div>
          ...
          <div style={{ padding:'0.875rem 1.75rem', borderTop:'1px solid #f1f5f9', display:'flex', alignItems:'center', justifyContent:'space-between', flexShrink:0 }}>
            ...
            <div style={{ display:'flex', gap:'0.625rem' }}>
              <button onClick={cerrar} disabled={cargando} style={{ padding:'0.5rem 1.125rem', fontSize:'0.8rem', fontWeight:500, borderRadius:'0.875rem', border:'1px solid #e2e8f0', color:'#64748b', background:'white', cursor: cargando ? 'not-allowed' : 'pointer', opacity: cargando ? 0.5 : 1 }}>Cancelar</button>
              <button disabled={!listo || cargando} onClick={cargarYCerrar}
                style={{ padding:'0.5rem 1.375rem', fontSize:'0.8rem', fontWeight:600, borderRadius:'0.875rem', border:'none', background: cargado?'#16a34a':listo&&!cargando?'#1e3a5f':'#e2e8f0', color: listo?'white':'#94a3b8', cursor: listo&&!cargando?'pointer':'not-allowed', transition:'background 0.3s' }}
                onMouseEnter={e=>{ if(listo&&!cargando&&!cargado) e.currentTarget.style.background='#2563eb' }} onMouseLeave={e=>{ if(listo&&!cargando&&!cargado) e.currentTarget.style.background='#1e3a5f' }}>
                {cargado ? '✓ Guardado' : cargando ? 'Guardando...' : `✓ Cargar ${listo?filasPaste.length:''} alumnos`}
              </button>
            </div>
          </div>
        </div>
...
export default function GruposPage() {
  ...
  async function handleGuardar(nombre: string, semestre: number) {
    try {
      setGuardando(true)
      await crearGrupo(nombre, semestre)
  ...
  async function handleEditar(id: string, nombre: string, semestre: number) {
    try {
      setGuardando(true)
      await editarGrupo(id, nombre, semestre)
    } catch (err) {
  ...
  async function handleCarga(grupoId: string, filas: AlumnoFila[]) {
    try {
      setCargandoAlumnos(true)
      await cargarAlumnos(grupoId, filas)
    } catch (err) {
  ...
  async function confirmarEliminar() {
    ...
    try {
      await eliminarGrupo(eliminando.id)
      cerrarEliminar()
    } catch (err) {
  ...
  return (
    <div className="flex flex-col h-full">
                              ...
                              <tr key={g.id} style={{ borderBottom:'1px solid #f8fafc' }}
                                ...
                                <td style={{ padding:'0.75rem 1rem', overflow:'visible' }}>
                                  <button onClick={() => setCargandoGrupo(g)}
                                    style={{ display:'flex', alignItems:'center', gap:'0.375rem', height:'28px', padding:'0 0.75rem', borderRadius:'9999px', border: g.total_alumnos && g.total_alumnos>0 ? '1px solid #16a34a' : '1px dashed #2563eb', background: g.total_alumnos && g.total_alumnos>0 ? '#f0fdf4' : '#eff6ff', cursor:'pointer', fontSize:'0.72rem', fontWeight:600, color: g.total_alumnos && g.total_alumnos>0 ? '#16a34a' : '#2563eb', transition:'all 0.15s', whiteSpace:'nowrap' }}
                                    onMouseEnter={e=>{ e.currentTarget.style.filter='brightness(0.95)' }}
                                    onMouseLeave={e=>{ e.currentTarget.style.filter='none' }}>
                                    {g.total_alumnos && g.total_alumnos > 0
                                      ? <><svg width="10" height="10" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M20 6L9 17l-5-5" strokeLinecap="round" strokeLinejoin="round"/></svg>{g.total_alumnos} alumnos</>
      ...
      {editando && (
        <ModalEditarGrupo grupo={editando} onGuardar={handleEditar} onCerrar={() => setEditando(null)} guardando={guardando} />
      )}
      {cargandoGrupo && (
        <ModalCargaAlumnos grupo={cargandoGrupo} onCargar={handleCarga} onCerrar={() => setCargandoGrupo(null)} cargando={cargandoAlumnos} />
      )}
